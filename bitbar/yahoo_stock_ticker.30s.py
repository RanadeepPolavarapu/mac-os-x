...
# Variables
indices_dict = {
    'SPX': {'regularMarketSymbol': '^GSPC', 'afterHoursSymbol': 'ES=F'},
}
GREEN = '\033[32m'
RED = '\033[31m'
RESET = '\033[0m'
...
# Checks if the current datetime is between Mon-Fri between 9:30 am to 4 pm.
def is_market_open():
    d = datetime.now()
    return d.isoweekday() in range(1, 6) and d.hour * 60 + d.minute in range(
        9 * 60 + 30, 16 * 60
    )
...
def check_price_limits(
    symbol_to_be_checked, current_price, price_limit_list, data_file
):
...
def print_index(index, name):
    ...
    change_points = index['regularMarketPrice'] - index['regularMarketPreviousClose']
    ...
    if market_state != 'REGULAR':
        ...
        colored_change_points = '(' + '{:.2f}'.format(change_points) + ') '
        colored_price = '🌛' + '{:.2f}'.format(index['regularMarketPrice'])
    if market_state == 'REGULAR':
        ...
        colored_change_points = (
            color + '(' + '{:.2f}'.format(change_points) + ') ' + RESET
        )
        colored_price = (
            color + ' ' + '{:.2f}'.format(index['regularMarketPrice']) + RESET
        )

    # Print the index info only to the menu bar
    # print(name, colored_change_points, colored_change, '| dropdown=false', sep=' ')
    print(colored_price, '| dropdown=false', sep=' ')
...
def print_stock(s):
    ...
    change_points = s['regularMarketPrice'] - s['regularMarketPreviousClose']
    ...
    fifty_day_change = (
        '(' + '{:.2f}'.format(s['fiftyDayAverageChangePercent'] * 100) + '%)'
    )
    two_hundred_day_change = (
        '(' + '{:.2f}'.format(s['twoHundredDayAverageChangePercent'] * 100) + '%)'
    )

    # Print the stock info seen in the dropdown menu
    stock_info = '{:<8} {:>10.2f} {:>20} {:>20}' + FONT
    print(
        stock_info.format(
            symbol, s['regularMarketPrice'], colored_change_points, colored_change
        )
    )
    ...
    print(
        stock_submenu.format(
            '--200 MA:', two_hundred_day + ' ' + two_hundred_day_change
        )
    )
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        for name, symbol in indices_dict.items():
            currentSymbol = (
                symbol['regularMarketSymbol']
                if is_market_open()
                else symbol['afterHoursSymbol']
            )
            # For each symbol: curl the data, then print it
            index = get_stock_data(currentSymbol)
        ...
        for symbol in symbols:
            ...
            stocks.append(stock)
            check_price_limits(
                symbol, stock['regularMarketPrice'], price_limit_list, data_file
            )
        ...
        if sort_by == 'market_change_winners':
            stocks = sorted(
                stocks, key=lambda k: k['regularMarketChangePercent'], reverse=True
            )
        ...
        if sort_by == 'market_change_volatility':
            stocks = sorted(
                stocks, key=lambda k: abs(k['regularMarketChangePercent']), reverse=True
            )
